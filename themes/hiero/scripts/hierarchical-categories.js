...
function hierarchicalCategoryListHelper(categories, options) {
	...
	var separator = options.hasOwnProperty('separator')
		? options.separator
	...
	function createHierarchicalCategoryList(level, parentId) {
		...
		prepareQuery(parentId).forEach(function(parent, i) {
			if (!depth || level + 1 < depth) {
				var childCategoryList = createHierarchicalCategoryList(
					level + 1,
					parent._id
				)
				childCategoryList.forEach(function(child) {
					list.push(
						new HierarchicalCategory(
							parent.name + separator + child.name,
							parent.posts
						)
					)
				})
	...
	// sort alphabetically
	var sortedCategoryList = categoryList.sort(function(a, b) {
...
hexo.extend.helper.register('hierarchicalCategoryList', function(
	categories,
	options
) {
